...
@implementation YMNetWorkAgent{
    AFHTTPSessionManager *_manager;
    YMNetWorkConfig *_config;
    dispatch_queue_t _processingQueue;
    //请求记录  字典
    NSMutableDictionary<NSNumber *, YMBaseRequest *> *_requestsRecord;
...
- (instancetype)init
{
    ...
    if (self) {
        //网络配置文件
        _config = [YMNetWorkConfig sharedInstance];
...
- (NSURLSessionTask *)sessionTaskForRequest:(YMBaseRequest *)request error:(NSError * _Nullable __autoreleasing *)error {
    ...
    switch (method) {
        ...
        case YMRequestMethodPOSTJSON:
            return [self dataTaskWithHTTPMethod:@"POSTJSON" requestSerializer:requestSerializer URLString:url parameters:param constructingBodyWithBlock:nil error:error];
...
- (NSURLSessionDataTask *)dataTaskWithHTTPMethod:(NSString *)method
    ...
    if (block) {
        ...
        if ([method isEqualToString:@"POSTJSON"]) {
            request = [[NSMutableURLRequest alloc]initWithURL:[NSURL URLWithString:URLString]];
            for (NSString *key in requestSerializer.HTTPRequestHeaders.allKeys) {
                NSString *value = [requestSerializer.HTTPRequestHeaders objectForKey:key];
...
- (void)handleRequestResult:(NSURLSessionTask *)task responseObject:(id)responseObject error:(NSError *)error {
    Lock();
    YMBaseRequest *request = _requestsRecord[@(task.taskIdentifier)];
    Unlock();
    if (!request) {
        return;
    }
...
//  生成请求url
- (NSString *)generateUrl:(YMBaseRequest *)request
{
...
// 生成请求参数
- (NSDictionary *)generatepParams:(YMBaseRequest *)request
{
...
- (AFHTTPRequestSerializer *)requestSerializerForRequest:(YMBaseRequest *)request {
    //默认是二进制请求
...
- (void)addRequestToRecord:(YMBaseRequest *)request {
...
- (void)removeRequestFromRecord:(YMBaseRequest *)request {
    Lock();
